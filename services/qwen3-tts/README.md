...
Alternative models remain supported through Kana's central
`tts.qwen3Local.model` configuration, but model capabilities and memory needs
must be verified independently.

## Install and run

Install [`uv`](https://docs.astral.sh/uv/) first. Select `qwen3-local` in
Kana's central `config.json`; Kana discovers this service, prepares an isolated
runtime, and starts it lazily on the first speech request. Storage, model,
device, dtype, attention, limits, and port are configured only under
`tts.qwen3Local`; see [the central configuration guide](../../docs/CONFIGURATION.md).
...
The Python process still receives generated `KANA_TTS_*` environment values,
but those are a private launch contract between Kana and this service—not a
second user configuration surface. Running the Python entry point directly is
supported only for service development and tests.
...
- On CPUs without BF16, set `tts.qwen3Local.dtype` to `float32` if `auto` ever
  mis-detects, and expect roughly 3× slower synthesis with higher memory
