...
export class TtsRelayProvider implements VoiceProvider {
  readonly id = "configured-tts-relay";
  ...
  constructor(
    private readonly options: TtsRelayProviderOptions,
  ...
  async inspect(): Promise<VoiceProviderStatus> {
    this.update({ state: "checking", message: "Checking the voice provider…" });
    const { status } = await inspectConfiguredTtsProvider();
    this.update({
      state:
        status.state === "ready"
          ? "ready"
  ...
  private async synthesizeChunk(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Voice provider request failed with HTTP ${response.status}.${detail}`,
        );
      }
  ...
  private async cancelUpstream(requestId: string, operation: number): Promise<void> {
    try {
      const response = await fetch(
        ttsRelayUrl(`/requests/${encodeURIComponent(requestId)}/cancel`),
      ...
      this.update({
        message: `Voice stopped locally, but the configured provider could not be told to cancel (${error instanceof Error ? error.message : "cancel failed"}).`,
      });
    }
  }
